...
async function main() {
  ...
  const vulnerabilityData = [
    ...
    { deviceId: devices[5].id, title: 'Default Credentials on CCTV System', description: 'CCTV camera at Bulawayo Platform A is using factory default username and password credentials, allowing unauthorized access to video feeds. Attackers could view, modify, or disable surveillance footage, compromising physical security monitoring at the station.', severity: 'critical', cvssScore: 9.1, cveId: 'CVE-2024-NRZ-002', status: 'open', discoveredDate: daysAgo(55), remediation: 'Change all default credentials immediately. Implement password rotation policy and multi-factor authentication for camera management interfaces.' },
    ...
    { deviceId: devices[22].id, title: 'Missing Network Segmentation on Kadoma Router', description: 'Router at Kadoma Station lacks proper VLAN segmentation, allowing lateral movement between IoT device networks and corporate network. An attacker compromising one device could pivot to all connected systems.', severity: 'high', cvssScore: 7.8, status: 'in_progress', discoveredDate: daysAgo(45), remediation: 'Implement VLAN segmentation. Deploy firewall rules to restrict traffic between network zones.' },
    { deviceId: devices[7].id, title: 'CCTV Camera Unauthenticated RTSP Stream', description: 'CCTV camera at Mutare Ticket Hall exposes an unauthenticated RTSP video stream. Anyone on the network can view live surveillance footage without credentials, potentially aiding physical intrusion planning.', severity: 'high', cvssScore: 8.2, cveId: 'CVE-2024-NRZ-007', status: 'open', discoveredDate: daysAgo(20), remediation: 'Enable RTSP authentication on all cameras. Implement stream encryption. Restrict RTSP access to monitoring VLAN only.' },
    { deviceId: devices[21].id, title: 'Edge Gateway Weak SNMP Community String', description: 'Edge gateway at Gweru uses default "public" SNMP community string, allowing attackers to read device configuration, enumerate network topology, and potentially modify device settings via SNMP write access.', severity: 'high', cvssScore: 7.5, status: 'acknowledged', discoveredDate: daysAgo(12), remediation: 'Change SNMP community strings to complex values. Disable SNMPv1/v2c and migrate to SNMPv3 with authentication and encryption.' },
    ...
    { deviceId: devices[9].id, title: 'CCTV Camera Unencrypted Video Storage', description: 'CCTV camera at Gweru Warehouse stores recorded footage without encryption on the local SD card. If the device is physically accessed, recorded video could be extracted and viewed.', severity: 'medium', cvssScore: 5.5, status: 'acknowledged', discoveredDate: daysAgo(25), remediation: 'Enable AES-256 encryption for local storage. Implement secure key management. Move to central NVR storage with encryption.' },
    { deviceId: devices[19].id, title: 'Gateway SNMP Information Disclosure', description: 'Harare Central Gateway reveals system information including firmware version, uptime, and connected devices through SNMP queries without requiring authentication.', severity: 'medium', cvssScore: 5.3, status: 'resolved', discoveredDate: daysAgo(60), resolvedDate: daysAgo(30), remediation: 'Configure SNMP access controls. Move to SNMPv3. Restrict SNMP queries to management workstation only.' },
    { deviceId: devices[10].id, title: 'CCTV Camera Outdated TLS Configuration', description: 'CCTV camera on Platform B at Harare Station supports only TLS 1.0 and TLS 1.1 which are deprecated and have known vulnerabilities. The management web interface uses these weak protocols.', severity: 'medium', cvssScore: 6.0, status: 'in_progress', discoveredDate: daysAgo(14), remediation: 'Update camera firmware to support TLS 1.2 minimum. Disable TLS 1.0 and 1.1 on all IoT management interfaces.' },
    { deviceId: devices[15].id, title: 'Vibration Sensor Insecure Data Transmission', description: 'The vibration sensor at Save River Bridge transmits structural monitoring data over HTTP without encryption. This data could be intercepted and manipulated, potentially masking structural damage alerts.', severity: 'medium', cvssScore: 5.7, status: 'open', discoveredDate: daysAgo(9), remediation: 'Implement HTTPS for data transmission. Deploy certificate pinning on sensor communications.' },

    // Low vulnerabilities (7)
    { deviceId: devices[0].id, title: 'Insufficient Logging and Monitoring', description: 'Signal controller at Harare Station does not generate adequate security logs, making it difficult to detect unauthorized access attempts or configuration changes.', severity: 'low', cvssScore: 4.3, status: 'resolved', discoveredDate: daysAgo(90), resolvedDate: daysAgo(40), remediation: 'Deploy centralized logging system with SIEM integration. Configure alert thresholds for suspicious activities.' },
  ...
  const solutionData = [
    ...
    { vulnerabilityId: vulnerabilities[18].id, title: 'Vibration Sensor Data Encryption Deployment', description: 'Implement HTTPS for vibration sensor data transmission at Save River Bridge. Deploy certificate pinning on sensor communications. Add data integrity verification to prevent manipulation of structural monitoring alerts.', implementationStatus: 'proposed', priority: 'medium', assignedTo: 'Rupondo Gumbo', dueDate: daysAgo(-10), costEstimate: '$8,000' },

    // Implemented solutions
    { vulnerabilityId: vulnerabilities[19].id, title: 'Centralized Logging and SIEM Deployment', description: 'Deploy centralized logging infrastructure with SIEM capabilities for all IoT devices. Configure automated alerting and reporting. Integration with existing network monitoring tools.', implementationStatus: 'implemented', priority: 'low', assignedTo: 'Tariro Moyo', completedDate: daysAgo(30), costEstimate: '$35,000' },
    { vulnerabilityId: vulnerabilities[16].id, title: 'SNMP Access Control Hardening - Harare Gateway', description: 'Configured SNMP access controls on Harare Central Gateway. Migrated from SNMPv2c to SNMPv3 with authentication and encryption. Restricted SNMP queries to management workstation only.', implementationStatus: 'implemented', priority: 'medium', assignedTo: 'Munyaradzi Chigama', completedDate: daysAgo(30), costEstimate: '$3,000' },
    { vulnerabilityId: vulnerabilities[21].id, title: 'Privacy Masking Configuration - CCTV Network', description: 'Configured privacy masking zones on all CCTV cameras to exclude non-monitoring areas. Applied consistent privacy zones based on NRZ surveillance policy requirements.', implementationStatus: 'implemented', priority: 'low', assignedTo: 'Tariro Moyo', completedDate: daysAgo(20), costEstimate: '$2,000' },

    // Low priority solutions
    { vulnerabilityId: vulnerabilities[22].id, title: 'Authenticated NTP Deployment', description: 'Configure authenticated NTP using symmetric keys on Kwekwe router. Point to trusted NRZ time servers only. Implement NTP monitoring to detect time synchronization anomalies.', implementationStatus: 'proposed', priority: 'low', assignedTo: 'Mufaro Zvobgo', dueDate: daysAgo(-60), costEstimate: '$1,500' },
    { vulnerabilityId: vulnerabilities[23].id, title: 'Asset Registration for Inactive GPS Tracker', description: 'Register inactive GPS tracker on Locomotive 03 in the asset management database. Perform full security review before reactivation. Update firmware to latest version and configure monitoring.', implementationStatus: 'proposed', priority: 'low', assignedTo: 'Mufaro Zvobgo', dueDate: daysAgo(-45), costEstimate: '$1,000' },
  ];
